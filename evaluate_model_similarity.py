...
def projection_hole_ratio(cells, grid=192):
    """Fraction of see-through cells enclosed by the silhouette of one projection.

    Occupancy IoU alone rewards hallucinated debris; this measures the defect
    reviewers actually reject - through-cavities you can see daylight through.
    """
    if not cells:
        return 0.0
    ...
    for index in range(grid):
        stack.extend(((index, 0), (index, grid - 1), (0, index), (grid - 1, index)))
    while stack:
        cell = stack.pop()
    ...
    interior_empty = grid * grid - len(visited) - len(cells)
    denominator = len(cells) + interior_empty
    return interior_empty / denominator if denominator else 0.0
...
def analyze(path: Path):
    ...
    return {
        ...
        "profiles": profiles,
        "occupancy": occ,
        # Half-resolution grid for hole detection: keeps coverage dense enough on
        # sparsely-triangulated flat panels that flood fill cannot leak through.
        "projection_holes": {
            name: projection_hole_ratio({(x // 2, y // 2) for x, y in cells}, grid=96)
            for name, cells in occ.items()
        },
...
def compare(reference, candidate):
    ...
    # Through-cavity delta: enclosed see-through holes the reference does not have.
    cavity_delta = max(
        max(0.0, candidate["projection_holes"][name] - reference["projection_holes"][name])
    ...
    return {
        ...
        "cavity_delta": round(cavity_delta, 5),
    }
...
def public_model_summary(model):
    return {
        ...
        "projection_holes": {k: round(v, 5) for k, v in model["projection_holes"].items()},
